"""Seed and run the synthetic evaluation suite.
...
from app.services.evaluation import run_answer_evaluation, run_retrieval_evaluation  # noqa: E402
...
DEFAULT_BENCHMARK = "phase8_synthetic_mixed_production"


def main() -> None:
    parser = argparse.ArgumentParser(description="Seed synthetic data and run retrieval/answer evaluations.")
    ...
    parser.add_argument("--limit", type=int, default=5, help="Retrieval limit per benchmark case.")
    args = parser.parse_args()

    init_db()
    db = SessionLocal()
    try:
        matter = _seed_dataset(db, Path(args.dataset), args.matter_name, args.matter_number)
        retrieval = run_retrieval_evaluation(db, matter_id=matter.id, dataset_name=args.benchmark, limit=args.limit)
        answer = run_answer_evaluation(db, matter_id=matter.id, dataset_name=args.benchmark, limit=args.limit)
        _print_summary("Retrieval", retrieval.metrics)
        _print_summary("Answer", answer.metrics)
    finally:
        db.close()


def _seed_dataset(db, dataset_path: Path, matter_name: str, matter_number: str) -> Matter:
    payload = json.loads(dataset_path.read_text(encoding="utf-8"))
    matter = _get_or_create_matter(db, matter_name, matter_number, payload.get("description"))
    with TemporaryDirectory() as tmp_dir:
        for document in payload.get("documents", []):
            asyncio.run(_ingest_document(db, matter, document, Path(tmp_dir)))
    print(f"Seeded {len(payload.get('documents', []))} documents into matter {matter.id}: {matter.name}")
    return matter
...
async def _ingest_document(db, matter: Matter, document: dict, tmp_dir: Path) -> None:
    ...
    with path.open("rb") as handle:
        ...
        await ingest_upload(db, upload, matter_id=matter.id, custodian_id=custodian.id if custodian else None)


def _print_summary(label: str, metrics) -> None:
    pass_metrics = [metric for metric in metrics if metric.metric_name.endswith("benchmark_pass")]
    pass_count = sum(1 for metric in pass_metrics if metric.metric_value == 1.0)
    print(f"{label}: {pass_count}/{len(pass_metrics)} benchmark cases passed")
    for metric in pass_metrics:
        print(f"  - {metric.case_id}: {metric.metric_value}")
